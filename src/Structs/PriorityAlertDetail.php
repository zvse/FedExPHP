...
class PriorityAlertDetail extends AbstractStructBase
{
     ...
     * @param string[] $content
     * @return \CommerceFedEx\FedExPHP\Structs\PriorityAlertDetail
     */
     ...
     * @uses \CommerceFedEx\FedExPHP\Enums\PriorityAlertEnhancementType::valueIsValid()
     * @uses \CommerceFedEx\FedExPHP\Enums\PriorityAlertEnhancementType::getValidValues()
     * @throws \InvalidArgumentException
     * @param string[] $enhancementTypes
     * @return \CommerceFedEx\FedExPHP\Structs\PriorityAlertDetail
     */
    public function setEnhancementTypes(array $enhancementTypes = array())
    {
        ...
        foreach ($enhancementTypes as $priorityAlertDetailEnhancementTypesItem) {
            if (!\CommerceFedEx\FedExPHP\Enums\PriorityAlertEnhancementType::valueIsValid($priorityAlertDetailEnhancementTypesItem)) {
                $invalidValues[] = var_export($priorityAlertDetailEnhancementTypesItem);
            }
        }
        if (!empty($invalidValues)) {
            throw new \InvalidArgumentException(sprintf('Value(s) "%s" is/are invalid, please use one of: %s', implode(', ', $invalidValues), implode(', ', \CommerceFedEx\FedExPHP\Enums\PriorityAlertEnhancementType::getValidValues())), __LINE__);
        }
    ...
    public function addToEnhancementTypes($item)
    {
        ...
        if (!\CommerceFedEx\FedExPHP\Enums\PriorityAlertEnhancementType::valueIsValid($item)) {
            throw new \InvalidArgumentException(sprintf('Value "%s" is invalid, please use one of: %s', $item, implode(', ', \CommerceFedEx\FedExPHP\Enums\PriorityAlertEnhancementType::getValidValues())), __LINE__);
        }
