...
class ProcessShipmentReply extends AbstractStructBase
{
    ...
    public function setNotifications(array $notifications = array())
    {
        foreach ($notifications as $processShipmentReplyNotificationsItem) {
            ...
            if (!$processShipmentReplyNotificationsItem instanceof \CommerceFedEx\FedExPHP\Structs\Notification) {
                throw new \InvalidArgumentException(sprintf('The Notifications property can only contain items of \CommerceFedEx\FedExPHP\Structs\Notification, "%s" given', is_object($processShipmentReplyNotificationsItem) ? get_class($processShipmentReplyNotificationsItem) : gettype($processShipmentReplyNotificationsItem)), __LINE__);
            }
        }
    ...
    public function setErrorLabels(array $errorLabels = array())
    {
        foreach ($errorLabels as $processShipmentReplyErrorLabelsItem) {
            // validation for constraint: itemType
            if (!$processShipmentReplyErrorLabelsItem instanceof \CommerceFedEx\FedExPHP\Structs\ShippingDocument) {
                throw new \InvalidArgumentException(sprintf('The ErrorLabels property can only contain items of \CommerceFedEx\FedExPHP\Structs\ShippingDocument, "%s" given', is_object($processShipmentReplyErrorLabelsItem) ? get_class($processShipmentReplyErrorLabelsItem) : gettype($processShipmentReplyErrorLabelsItem)), __LINE__);
            }
        }
